...
def test_follow(client):
    ...
    data = client.post(
        url_for('user.follow', username=user.username), follow_redirects=True
    ...
    data = client.post(
        url_for('user.unfollow', username=user.username), follow_redirects=True
    ).get_data(as_text=True)
...
def test_all_users(client):
    login(client)
    response = client.get(url_for('user.all_users'))
...
def test_change_password_with_auth(client):
    ...
    response = client.post(url_for('user.change_password'), data=form_data, follow_redirects=True)
    assert response.status_code == 200
    assert admin.verify_password('abcd1234')


def test_reset_password_without_auth(client):
    random_user_id = randint(1, User.query.count())
    user = User.query.get(random_user_id)
    with mail.record_messages() as outbox:
        response = client.post(
            url_for('user.forget_password'),
    ...
    response = client.post(
        url_for('user.reset_password', token=token),
