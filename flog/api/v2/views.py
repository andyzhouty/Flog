...
# get user schema
api_v2.add_url_rule('/user/<int:user_id>/', view_func=UserAPI.as_view('user'),
                    methods=['GET', 'PUT', 'DELETE'])
# get/put/patch/delete a post
api_v2.add_url_rule('/post/<int:post_id>/', view_func=PostAPI.as_view('post'),
                    methods=['GET', 'PUT', 'PATCH', 'DELETE'])
api_v2.add_url_rule('/post/new/', view_func=PostAPI.as_view('new_post'),
                    methods=['POST'])
# collect
api_v2.add_url_rule('/post/<any(collect,uncollect):collect_or_uncollect>/<int:post_id>/',
                    view_func=CollectionAPI.as_view('collect'),
                    methods=['GET'])
# follow
api_v2.add_url_rule('/user/any(follow,unfollow):follow_or_unfollow/<int:user_id>/',
                    view_func=FollowAPI.as_view('follow'), methods=['GET'])
# comment
api_v2.add_url_rule('/comment/<int:comment_id>/',
